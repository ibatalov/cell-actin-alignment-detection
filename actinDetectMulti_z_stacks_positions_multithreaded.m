...
inputTitles = {'number of samples:', 'positions', 'channels', 'slices (max.)', 'ouput file title:','actin layer number:','alpha-actinin layer number:'};
...
output_strings = cell(SampleCount, 1);

% determining thresh for later use in parfor loop
if ~exist('thresh','var')
    picturename = [FileList{1,1} FileList{1,2}];
    ...
    actin = double(img{1,1}{actinLayerNumber,1});
...
parfor i=1:SampleCount
    ...
    for j = 1:PictureCount
        ...
        for position = 1 : numberOfPositions
            ...
            current_numberOfSlices = size(img{position, 1}, 1)/numberOfChannels;
            
            if(actininLayerNumber >= 0)
                mip = MIP(img{position, 1}(:, 1), numberOfChannels, [actinLayerNumber, actininLayerNumber]);
                maxActinPx = max(mip{1}(:));
                maxSarcomerePx = max(mip{2}(:));
            else
            ...
            for sliceNum = 1 : current_numberOfSlices
...
%                     end
                    ...
                    output_strings{i} = [output_strings{i}, sprintf([picture ';%3i;%3i;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;;'], position, sliceNum - slice_shift, sarcomere_area_fraction, PercentActinArea, Mean, Std, Median, Mode, cms_oop)];
    ...
    for sliceNum = 1 : size(All_angles, 1) + 1
        ...
        if(sliceNum <= size(All_angles, 1))
            ...
            if(numel(average_sarcomere_area{sliceNum}) > 0)
        ...
        if(analyzeFibroblasts == 1)
            ...
            %Save data
            if(sliceNum <= size(All_angles, 1))
                output_strings{i} = [output_strings{i}, sprintf('Average;all;%3i;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;;', sliceNum, average_sarcomeres, average_actin, Mean, Std, Median, Mode, OrientationOrderParameter)];
                output_strings{i} = [output_strings{i}, sprintf('Average_fbs;all;%3i;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f\n', sliceNum, average_fbs_actin, fbs_Mean, fbs_Std, fbs_Median, fbs_Mode, fbs_OrientationOrderParameter)];
            else
                output_strings{i} = [output_strings{i}, sprintf('Average;all;all;no;no;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;;', Mean, Std, Median, Mode, OrientationOrderParameter)];
                output_strings{i} = [output_strings{i}, sprintf('Average_fbs;all;all;no;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f\n', fbs_Mean, fbs_Std, fbs_Median, fbs_Mode, fbs_OrientationOrderParameter)];
            end
            
        else
            if(sliceNum <= size(All_angles, 1))
                output_strings{i} = [output_strings{i}, sprintf('Average;all;%3i;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f\n', sliceNum, average_sarcomeres, average_actin, Mean, Std, Median, Mode, OrientationOrderParameter)];
            else
                output_strings{i} = [output_strings{i}, sprintf('Average;all;all;no;no;%3.3f;%3.3f;%3.3f;%3.3f;%3.3f\n', Mean, Std, Median, Mode, OrientationOrderParameter)];
            end
        end
...
for i = 1 : SampleCount
    fprintf(fileID, output_strings{i});
end
fclose('all');
clearvars thresh;
